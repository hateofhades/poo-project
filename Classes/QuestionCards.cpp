#include "QuestionCards.h"

QuestionCards::QuestionCards(string question, int correctAnswer, sf::Font font, sf::Sprite image, string answer1, string answer2, string answer3)
{
    this->font = font;
    this->image = image;
    ...
    this->box1.setSize(sf::Vector2f(this->background.getSize().x / 2 - 100, this->background.getSize().y / 15));
    this->box1.setPosition(this->background.getPosition().x + 283, this->background.getPosition().y + this->questionTxt.getLocalBounds().height + this->box1.getSize().y + this->image.getGlobalBounds().height + 10);
    ...
    this->box2.setSize(sf::Vector2f(this->background.getSize().x / 2 - 100, this->background.getSize().y / 15));
    this->box2.setPosition(this->background.getPosition().x + 283, this->background.getPosition().y + this->questionTxt.getLocalBounds().height + 10 + this->image.getGlobalBounds().height + this->box1.getSize().y + 10 + this->box2.getSize().y);
    ...
    this->box3.setSize(sf::Vector2f(this->background.getSize().x / 2 - 100, this->background.getSize().y / 15));
    this->box3.setPosition(this->background.getPosition().x + 283, this->background.getPosition().y + this->questionTxt.getLocalBounds().height + 10 + this->image.getGlobalBounds().height + this->box1.getSize().y + 10 + this->box2.getSize().y + 10 + this->box3.getSize().y);
...
void QuestionCards::selectAnswer(int number)
{
...
int QuestionCards::getSelected()
{
    return this->selectedAnswer;
};

void QuestionCards::setHint(string text)
{
    this->hint.setString(text);
    this->hint.setPosition(this->background.getPosition().x + (this->background.getSize().x - this->hint.getLocalBounds().width) / 2, this->background.getPosition().y + this->background.getSize().y - this->hint.getLocalBounds().height - 10);
};

string QuestionCards::getQuestion()
{
    return this->question;
};

string QuestionCards::getAnswer(int number)
{
    switch (number)
    {
    case 1:
...
int QuestionCards::getCorrectAnswer()
{
    return this->correctAnswer;
};

void QuestionCards::setBackgroundSize(int width, int height)
{
    this->background.setSize(sf::Vector2f(width, height));
};

void QuestionCards::draw(sf::RenderTarget &target, sf::RenderStates states) const
{
