#include "QuestionWordSearch.h"

using namespace std;

QuestionWordSearch::QuestionWordSearch(string question, string correctAnswer, sf::Font font)
{
    this->hasPhoto = 0;
    this->font = font;
...
void QuestionWordSearch::setInputAnswer(string inputAnswer)
{
    this->inputAnswer.setString(inputAnswer);
    this->inputAnswer.setPosition(this->input.getPosition().x + (this->input.getSize().x - this->inputAnswer.getLocalBounds().width) / 2, this->input.getPosition().y + (this->input.getSize().y - this->inputAnswer.getLocalBounds().height) / 2);
}

string QuestionWordSearch::getInputAnswer()
{
    return this->inputAnswer.getString();
}

string QuestionWordSearch::getCorrectAnswer()
{
    return this->correctAnswer;
}

void QuestionWordSearch::addPhoto(string photoPath)
{
    this->hasPhoto = 1;
...
void QuestionWordSearch::draw(sf::RenderTarget &target, sf::RenderStates states) const
{
...
void QuestionWordSearch::changeBackgroundSize(int width, int height)
{
